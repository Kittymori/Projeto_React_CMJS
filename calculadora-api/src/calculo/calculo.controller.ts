...
export class CalculoController {
    ...
    async simularCalculo(@Body() dados: CalculoDto) {
        ...
        // A validação por DTO
        if (renda === undefined || custos === undefined || renda < 0 || custos < 0) {
             throw new HttpException('Renda e Custos são obrigatórios e devem ser números positivos.', HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            return {
             ...
             throw new HttpException('Falha interna ao calcular tributação. Verifique o console do servidor.', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}